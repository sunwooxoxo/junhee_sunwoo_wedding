...
(function () {
 ...
 function initCountdown() {
  ...
  function update() {
    ...
    const weddingDay = new Date(CONFIG.wedding.date + 'T00:00:00');
    weddingDay.setHours(0, 0, 0, 0);

    const totalDays = Math.round(
      (weddingDay - today) / (1000 * 60 * 60 * 24)
    );

    labelEl.textContent = `결혼식까지 D-${totalDays}`;

    // 시간/분/초는 실제 결혼식 시간 기준
...
function initLocation() {
  ...
  // 클릭했을 때 크게 보여줄 고화질 약도
  const mapLargeSrc = 'images/location/map-large.jpg';
